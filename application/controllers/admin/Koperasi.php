...
class Koperasi extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->model(['M_Koperasi', 'M_Logging']);
	...
	public function index()
	{
		$data = [
			'title' => 'Invoice',
			'pages' => 'dashboard/pages/invoice-koperasi/v_invoice',
			'invoices' => $this->M_Koperasi->list_invoice(),
			'customers' => $this->M_Koperasi->list_customer(),
	...
	public function add()
	{
		$customer = $this->M_Koperasi->show_customer($this->input->post('customer'));
		$max_num = $this->M_Koperasi->select_max();
		...
		$data = [
			...
			'pages' => 'dashboard/pages/invoice-koperasi/v_add_invoice',
			'no_invoice' => $no_inv,
			'customers' => $this->M_Koperasi->list_customer(),
			'customer' => $customer,
			'user' => $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array()
	...
	public function store()
	{
		...
		if ($old_slug) {
			$inv = $this->M_Koperasi->show($old_slug);

			$total_detail = $this->M_Koperasi->sum_total($inv['Id']);
			...
			$this->M_Koperasi->update_invoice($inv['Id'], $data);
			...
			$id_invoice = $this->M_Koperasi->insert($invoice_data);
			...
			if (is_array($menus)) {
				...
				if (!empty($detail_data)) {
					$insert = $this->M_Koperasi->insert_batch($detail_data);
	...
	public function update_item($id_invoice, $id)
	{
		...
		$this->M_Koperasi->update_item($id, $data);
		$this->add_log('update', $id, 'invoice_details');

		// update invoice setelah hapus row
		$diskon = $this->M_Koperasi->get_discount($id);

		$total_detail = $this->M_Koperasi->sum_total($id_invoice);
		...
		$this->M_Koperasi->update_invoice($id_invoice, $data);
	...
	{
		$this->M_Koperasi->delete_detail($id);
	...
	public function print($no_inv)
	{
		...
		$html = $this->load->view('dashboard/pages/invoice-koperasi/v_invoice_pdf', $data, true);
	...
	public function report()
	{
		...
		$invoices = $this->M_Koperasi->report($from, $to);
	...
	{
		$invoice = $this->M_Koperasi->show($no_inv);
		...
		if (!$invoice) {
			...
			if (is_array($menus)) {
				...
				if (!empty($detail_data)) {
					...
					if ($insert) {
						// update invoice setelah hapus row
						$diskon = $this->M_Koperasi->get_discount($id_inv);

						$total_detail = $this->M_Koperasi->sum_total($id_inv);
						...
						$this->M_Koperasi->update_invoice($id_inv, $data);

						$this->add_log('update', $id_inv, 'invoice');
	...
	public function customer()
	{
		$data = [
			...
			"pages" => "dashboard/pages/invoice-koperasi/v_customer",
			"customers" => $this->M_Koperasi->list_customer(),
